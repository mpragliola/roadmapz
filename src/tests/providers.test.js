...
describe('PROVIDER_REGISTRY', () => {
  ...
  it('each entry has createProvider, defaultModel, and keyVar', () => {
    for (const [name, entry] of Object.entries(PROVIDER_REGISTRY)) {
      ...
      expect(entry.keyVar, `${name}.keyVar`).toMatch(/_API_KEY$/);
    }
  });
});
...
describe('resolveConfig', () => {
  it('returns error when AI_PROVIDER is missing', () => {
    ...
    expect(result.error).toMatch(/AI_PROVIDER/);
  });

  it('returns error when AI_PROVIDER is unknown', () => {
    const result = resolveConfig({ AI_PROVIDER: 'unknown_provider' });
    expect(result.error).toMatch(/AI_PROVIDER/);
  });

  it('returns error naming the missing key var', () => {
    const result = resolveConfig({ AI_PROVIDER: 'anthropic' });
    expect(result.error).toMatch(/ANTHROPIC_API_KEY/);
  });

  it('returns config with provider default model when AI_MODEL is absent', () => {
    const result = resolveConfig({
